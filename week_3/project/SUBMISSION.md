...
Agent Class Hierarchy and Tool Dispatch -> The core Agent class handles the entire LLM lifecycle (_run_loop), session management, and routing tool calls via dispatch(). The dispatch() method  routes OpenAI tool requests to the Python functions mapped in the ALL_TOOLS_REGISTRY and handles execution errors. Because Agent contains all this logic, the UI classes (REPLAgent and TUIAgent) simply inherit from it and only override the presentation layer (the run() method and the _emit() event hook), completely avoiding duplicate chat loops or tool execution code.